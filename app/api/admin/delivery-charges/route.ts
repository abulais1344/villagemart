...
export async function POST(request: NextRequest) {
  ...
  // Guard: only one active flat-charge row per merchant_type (DB unique index enforces this too,
  // but we surface a clear message here rather than letting the constraint bubble up raw).
  if (payload.merchant_type) {
    ...
    if (existing) {
      return NextResponse.json(
        { error: `An active flat charge for '${payload.merchant_type}' already exists. Deactivate or delete it first.` },
        { status: 409 },
      );
    }
  }
